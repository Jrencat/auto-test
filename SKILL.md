...
**输出**
- 增量维护的 TestCase：`docs/testcases/<module>/`（含多分支页面的 `VARIANT_数据变体矩阵用例.md`）
...
统一入口：`/auto-test [路径...]`（或直接描述测试意图触发本 Skill）。
...
| `templates/testcase-e2e.md` / `testcase-api.md` / `testcase-import.md` | 各类 TestCase 模板 |
